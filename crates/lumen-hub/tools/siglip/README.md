# Lumen Hub SigLIP Model

SigLIP 双编码器推理服务：文本嵌入和图像嵌入。
...
- `precision`: `fp32`, `fp16`, `int8` 等
- `ext`: `onnx`（ONNX Runtime）或 `rknn`（RKNN Runtime）
- `model_info.json` 位于仓库根目录，是 SigLIP 运行时元数据的唯一来源
- `tokenizer.json` 是文本任务运行所需的 tokenizer artifact

### model_info.json

完整最小示例见 [`model_info.example.json`](./model_info.example.json)。该文件是 SigLIP
服务 `model_info.json` 示例的 SSOT；README 只描述字段契约，不维护重复 JSON 示例。

## task_metadata 字段

位于 `model_info.json` 的 `task_metadata` 字段，SigLIP 服务解析以下结构：

| 字段 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `tasks` | `map<string, object>` | 是 | 任务定义，key 为任务名（如 `semantic_text_embed`, `semantic_image_embed`） |
| `embedding_dim` | `usize` | 否 | embedding 维度，仅作为描述性元数据；运行时不依赖它 |
...
| `output_name` | `string` | 是 | ONNX 模型 forward-pass 输出的原始 embedding 节点名（在 L2 归一化之前），如 `"text_features"` 或 `"image_features"` |
| `hidden_output_name` | `string` | 否 | ONNX 模型 token/patch hidden states 输出名，如 `"last_hidden_state"`；当前 semantic embedding 运行时不消费 |
...
| 预处理 | HuggingFace `tokenizers` 库进行 tokenization -> `input_ids` + 可选 `attention_mask` |
...
| 输出 MIME | `application/json;schema=embedding_v1` |
| 预处理 | `model_info.json` 的 `task_metadata.tasks.<image_task>.preprocess`：resize + 可选 center crop + rescale + normalize |
...
ServiceConfig.models[alias]
  ...
  |
  |-- 2. 解析 task_metadata.tasks (BTreeMap<String, SiglipTaskConfig>)
  |
  |-- 3. For each task:
  |       |-- 按 component 分发
...
runtime/component/precision 三元组直接映射到文件系统路径。如果需要其他命名约定，应当在 `ModelConfig` 或 `SiglipTaskConfig` 中增加可配置字段。
