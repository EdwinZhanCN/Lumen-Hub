...
async fn exported_clip_model_latency() {
    let model_dir = env::var("LUMEN_CLIP_E2E_MODEL_DIR")
        .or_else(|_| env::var("LUMEN_CLIP_LATENCY_MODEL_DIR"))
        .map(PathBuf::from)
    ...
    let model_name = model_dir
        ...
        .to_string_lossy()
        .into_owned();
    let precision = env::var("LUMEN_CLIP_E2E_PRECISION")
        .or_else(|_| env::var("LUMEN_CLIP_LATENCY_PRECISION"))
        .unwrap_or_else(|_| "fp32".to_owned());
    let warmup = env_usize("LUMEN_CLIP_LATENCY_WARMUP", 5);
    let iters = env_usize("LUMEN_CLIP_LATENCY_ITERS", 30);

    let accelerated =
        env_bool("LUMEN_CLIP_LATENCY_ACCELERATED") || env_bool("LUMEN_CLIP_E2E_ACCELERATED");
    ...
    println!("text_embed {}", text);
    println!("image_embed {}", image);
}
