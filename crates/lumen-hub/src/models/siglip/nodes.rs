...
///
/// Inputs may be `Float32` or `Float16`. Computation is performed in `f32`;
/// output defaults to `Float32` because service responses serialize embeddings
/// as `EmbeddingV1`.
...
impl L2NormalizeNode {
    ...
    pub fn with_epsilon_and_output_dtype(
        ...
        if *descriptor.shape.last().expect("shape is non-empty") == 0 {
        ...
        Ok(Self {
            ...
            input_descriptors: HashMap::from([(input_name, descriptor)]),
...
impl MLNode for L2NormalizeNode {
    ...
    async fn execute(
        ...
        if !inputs.is_empty() {
            ...
            names.sort();
            return Err(format!("unexpected inputs provided: {}", names.join(", ")));
        }
        ...
        let tensor = match output_desc.dtype {
            ...
            other => return Err(format!("L2NormalizeNode cannot emit dtype {other:?}")),
        };
        let packet = MLPacket::from_host_tensor(context, output_desc, tensor)?;

        Ok(HashMap::from([(self.output_name.clone(), packet)]))
    }
}
