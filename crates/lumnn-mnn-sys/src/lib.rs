...
mod normal_impl {
    ...
    impl InferenceEngine {
        ...
        pub fn from_buffer(model_buffer: &[u8], config: Option<InferenceConfig>) -> Result<Self> {
            ...
            let (input_names, output_names, input_shapes, output_shapes, first_input, first_output) =
                unsafe { Self::get_io_info(ptr.as_ptr())? };

            Ok(InferenceEngine {
                ptr,
                input_shape: first_input,
                output_shape: first_output,
                input_names,
                output_names,
                input_shapes,
                output_shapes,
        ...
        unsafe fn get_io_info(
            ...
            let input_count = ffi::mnnr_get_input_count(ptr);
            let output_count = ffi::mnnr_get_output_count(ptr);
            ...
            for i in 0..input_count {
                let name_ptr = ffi::mnnr_get_input_name_at(ptr, i);
                if name_ptr.is_null() {
                    return Err(MnnError::RuntimeError(format!(
                        "Failed to get input name at index {i}"
                    )));
                }
                let name = CStr::from_ptr(name_ptr).to_string_lossy().into_owned();

                let mut dims = vec![0usize; 8];
                let mut ndims = 0;
                if ffi::mnnr_get_input_shape_at(ptr, i, dims.as_mut_ptr(), &mut ndims)
                    != ffi::MNNR_ErrorCode_MNNR_SUCCESS
                {
                    return Err(MnnError::RuntimeError(format!(
                        "Failed to get input shape at index {i}"
                    )));
                }
                dims.truncate(ndims);
            ...
            for i in 0..output_count {
                let name_ptr = ffi::mnnr_get_output_name_at(ptr, i);
                if name_ptr.is_null() {
                    return Err(MnnError::RuntimeError(format!(
                        "Failed to get output name at index {i}"
                    )));
                }
                let name = CStr::from_ptr(name_ptr).to_string_lossy().into_owned();
                ...
                let mut dims = vec![0usize; 8];
                let mut ndims = 0;
                if ffi::mnnr_get_output_shape_at(ptr, i, dims.as_mut_ptr(), &mut ndims)
                    != ffi::MNNR_ErrorCode_MNNR_SUCCESS
                {
                    return Err(MnnError::RuntimeError(format!(
                        "Failed to get output shape at index {i}"
                    )));
                }
                dims.truncate(ndims);
            ...
            // Backward compat: first input/output shapes
            let first_input = input_shapes.first().cloned().unwrap_or_default();
            let first_output = output_shapes.first().cloned().unwrap_or_default();
        ...
        /// Input tensor names in order.
        pub fn input_names(&self) -> &[String] {
            &self.input_names
        }

        /// Output tensor names in order.
        pub fn output_names(&self) -> &[String] {
            &self.output_names
        }

        /// Shape of the input tensor at the given index.
        pub fn input_shape_at(&self, index: usize) -> Option<&[usize]> {
            self.input_shapes.get(index).map(|s| s.as_slice())
        }

        /// Shape of the output tensor at the given index.
        pub fn output_shape_at(&self, index: usize) -> Option<&[usize]> {
            self.output_shapes.get(index).map(|s| s.as_slice())
        }

        /// Shape of the named input tensor as currently known by MNN.
        pub fn input_shape_by_name(&self, name: &str) -> Result<Vec<usize>> {
            let name_c = CString::new(name)
                .map_err(|_| MnnError::InvalidParameter("name has null byte".into()))?;
            let mut dims = vec![0usize; 8];
            let mut ndims = 0usize;
            let code = unsafe {
                ffi::mnnr_get_input_shape_by_name(
                    self.ptr.as_ptr(),
                    name_c.as_ptr(),
                    dims.as_mut_ptr(),
                    &mut ndims,
            ...
            match code {
                ...
                _ => Err(MnnError::InvalidParameter(get_last_error_message(Some(
                    self.ptr.as_ptr(),
                )))),
            }
        }

        /// Shape of the named output tensor as currently known by MNN.
        pub fn output_shape_by_name(&self, name: &str) -> Result<Vec<usize>> {
            let name_c = CString::new(name)
                .map_err(|_| MnnError::InvalidParameter("name has null byte".into()))?;
            let mut dims = vec![0usize; 8];
            let mut ndims = 0usize;
            let code = unsafe {
                ffi::mnnr_get_output_shape_by_name(
                    self.ptr.as_ptr(),
                    name_c.as_ptr(),
                    dims.as_mut_ptr(),
                    &mut ndims,
        ...
        /// Resize a named input tensor and refresh MNN's session I/O tensors.
        pub fn resize_input_by_name(&self, name: &str, shape: &[usize]) -> Result<()> {
            let name_c = CString::new(name)
                .map_err(|_| MnnError::InvalidParameter("name has null byte".into()))?;
            let code = unsafe {
                ffi::mnnr_resize_input_by_name(
                    self.ptr.as_ptr(),
                    name_c.as_ptr(),
                    shape.as_ptr(),
                    shape.len(),
        ...
        /// Halide type code of the input at index.
        pub fn input_dtype_at(&self, index: usize) -> i32 {
            unsafe { ffi::mnnr_get_input_dtype_at(self.ptr.as_ptr(), index) }
        }

        /// Halide type code of the output at index.
        pub fn output_dtype_at(&self, index: usize) -> i32 {
            unsafe { ffi::mnnr_get_output_dtype_at(self.ptr.as_ptr(), index) }
        }

        /// Query the actual runtime backend type of the output tensor.
        ...
        pub fn backend_type(&self) -> i32 {
            unsafe { ffi::mnnr_get_backend_type(self.ptr.as_ptr()) }
        }

        /// Get the model's input tensor name.
        pub fn input_name(&self) -> Option<String> {
            unsafe {
                let ptr = ffi::mnnr_get_input_name(self.ptr.as_ptr());
                if ptr.is_null() {
                    None
                } else {
        ...
        pub fn output_name(&self) -> Option<String> {
            unsafe {
                let ptr = ffi::mnnr_get_output_name(self.ptr.as_ptr());
                if ptr.is_null() {
                    None
                } else {
                    Some(CStr::from_ptr(ptr).to_string_lossy().into_owned())
        ...
        /// Execute inference with multiple named inputs and outputs.
        ///
        /// Each input and output is a named f32 slice. The engine copies all inputs,
        /// runs the session once, and copies all outputs.
        pub fn run_multi(
            &self,
            inputs: &[(&str, &[f32])],
            outputs: &mut [(&str, &mut [f32])],
        ) -> Result<()> {
            let in_names_c = inputs
                .iter()
                .map(|(name, _)| {
                    CString::new(*name)
                        .map_err(|_| MnnError::InvalidParameter("input name has null byte".into()))
                })
                .collect::<Result<Vec<_>>>()?;
            let out_names_c = outputs
                .iter()
                .map(|(name, _)| {
                    CString::new(*name)
                        .map_err(|_| MnnError::InvalidParameter("output name has null byte".into()))
                })
                .collect::<Result<Vec<_>>>()?;
            let in_names: Vec<_> = inputs
                .iter()
                .enumerate()
                .map(|(i, _)| in_names_c[i].as_ptr())
                .collect();
            let in_data: Vec<_> = inputs.iter().map(|(_, d)| d.as_ptr()).collect();
            let in_sizes: Vec<_> = inputs.iter().map(|(_, d)| d.len()).collect();

            let out_names: Vec<_> = outputs
                .iter()
                .enumerate()
                .map(|(i, _)| out_names_c[i].as_ptr())
                .collect();
            let mut out_data: Vec<_> = outputs.iter_mut().map(|(_, d)| d.as_mut_ptr()).collect();
            let out_sizes: Vec<_> = outputs.iter().map(|(_, d)| d.len()).collect();
        ...
        /// Copy f32 input data to a named tensor (without running).
        pub fn copy_input_f32(&self, name: &str, data: &[f32]) -> Result<()> {
            let name_c = CString::new(name)
                .map_err(|_| MnnError::InvalidParameter("name has null byte".into()))?;
            let code = unsafe {
                ffi::mnnr_copy_input_f32(
                    self.ptr.as_ptr(),
                    name_c.as_ptr(),
                    data.as_ptr(),
                    data.len(),
        ...
        /// Copy i32 input data to a named tensor (without running).
        pub fn copy_input_i32(&self, name: &str, data: &[i32]) -> Result<()> {
            let name_c = CString::new(name)
                .map_err(|_| MnnError::InvalidParameter("name has null byte".into()))?;
            let code = unsafe {
                ffi::mnnr_copy_input_i32(
                    self.ptr.as_ptr(),
                    name_c.as_ptr(),
                    data.as_ptr(),
                    data.len(),
        ...
        /// Copy i64 input data to a named tensor (without running).
        pub fn copy_input_i64(&self, name: &str, data: &[i64]) -> Result<()> {
            let name_c = CString::new(name)
                .map_err(|_| MnnError::InvalidParameter("name has null byte".into()))?;
            let code = unsafe {
                ffi::mnnr_copy_input_i64(
                    self.ptr.as_ptr(),
                    name_c.as_ptr(),
                    data.as_ptr(),
                    data.len(),
        ...
        /// Run inference (inputs must be copied first).
        pub fn run_only(&self) -> Result<()> {
            let code = unsafe { ffi::mnnr_run(self.ptr.as_ptr()) };
            match code {
                ffi::MNNR_ErrorCode_MNNR_SUCCESS => Ok(()),
                _ => Err(MnnError::RuntimeError(get_last_error_message(Some(
                    self.ptr.as_ptr(),
        ...
        /// Copy f32 output data from a named tensor (after running).
        pub fn copy_output_f32(&self, name: &str, data: &mut [f32]) -> Result<()> {
            let name_c = CString::new(name)
                .map_err(|_| MnnError::InvalidParameter("name has null byte".into()))?;
            let code = unsafe {
                ffi::mnnr_copy_output_f32(
                    self.ptr.as_ptr(),
                    name_c.as_ptr(),
                    data.as_mut_ptr(),
                    data.len(),
        ...
        /// Copy i32 output data from a named tensor (after running).
        pub fn copy_output_i32(&self, name: &str, data: &mut [i32]) -> Result<()> {
            let name_c = CString::new(name)
                .map_err(|_| MnnError::InvalidParameter("name has null byte".into()))?;
            let code = unsafe {
                ffi::mnnr_copy_output_i32(
                    self.ptr.as_ptr(),
                    name_c.as_ptr(),
                    data.as_mut_ptr(),
                    data.len(),
        ...
        /// Copy i64 output data from a named tensor (after running).
        pub fn copy_output_i64(&self, name: &str, data: &mut [i64]) -> Result<()> {
            let name_c = CString::new(name)
                .map_err(|_| MnnError::InvalidParameter("name has null byte".into()))?;
            let code = unsafe {
                ffi::mnnr_copy_output_i64(
                    self.ptr.as_ptr(),
                    name_c.as_ptr(),
                    data.as_mut_ptr(),
                    data.len(),
